...
The normal-normal family of Bayesian models uses a Gaussian distribution to represent both the prior and likelihood functions. This family is well-suited to modeling continuous data, particularly where there are no upper or lower bounds on the values the data or parameter can take on. Our target of inference is $\mu$.
...
The normal distribution is the most common distribution used in statistical modeling. In cases where we know the mean and typical variance in our data, and where we expect the data we sample to be symmetrically distributed on either side of the mean, many different random processes result in a normal sampling distribution. The symmetry is convenient, because if we can specify the mean of the distribution, we know that will also be equivalent to both the median and the mode. Also convenient is the fact that the mean and variance are defined independently of each other, allowing us to freely adjust the shape and location of our distribution as needed.
...
Normal distributions are useful for modeling the distribution of data which is continuous, unbounded, and symmetrically distributed around its mean, which leads to it being used to represent most continuous variables in statistics. For example, in our ChickWeight dataset, the final weights of our chicks are normally-distributed, with a mean of `r signif(mean(subset(ChickWeight, Time == 21)$weight), 3)` and a standard deviation of `r signif(sd(subset(ChickWeight, Time == 21)$weight), 3)`:
...
From our prior research (https://www.agrifarming.in/poultry-feed-chart-and-weight-chart#layer-poultry-feed-chart-and-weight-chart), the mean body weight for layer poultry (chickens that will be used for egg production) is typically around 190 grams at three weeks old. The variance of the distribution shows the confidence we have in our prior - higher variance means we are less confident and $\mu$ could reasonably cover a larger range of values, while a smaller variance means we are more confident in our estimate. For this example, we use 47.5 grams as our standard deviation. Because this is 1/4th of the mean, it is not a very strong prior, and our data will strongly influence our posterior estimate of $\mu$.
...
This plot represents the observed end weights and standard deviation. The end weight observed in our data would most likely have arisen if the mean end weight across all chicks were between 215 to 225 grams. We most likely observe a mean weight of 218.69 grams among our 45 sample subjects if the underlying population mean is also 218.69 grams.  
...
Here we can see a prior, likelihood, and a posterior distributions. Using our data, with the prior using the data information from the averages and the standard deviation from the population. The likelihood using the distributions from our data to get eh posterior. This graph shows how that the mean of likelihood distribution has a larger influence than the prior distribution, especially when it is a non-informative prior.
