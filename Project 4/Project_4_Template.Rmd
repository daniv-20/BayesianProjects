...
curve(dnorm(x, mu, sigma), from = mu-3*sigma, to=mu+3*sigma, axes = FALSE, xlab = "Beta1", ylab = "Density", main = "Prior Distribution for Centered Beta1 \n(Theoretical Prior)")
...
rstan identified a prior of $\mu_1$ = `r signif(p_sum$prior$location[1],3)` and $\sigma_1$ = `r signif(sig1,3)` for the priorfrac coefficient. rstan identified a prior of $\mu_2$ = `r signif(p_sum$prior$location[2],3)` and $\sigma_2$ = `r signif(sig2,3)` for the age coefficient.  
...
Where $ln[\frac{\pi_i}{1 - \pi_i}]$ represents the log-odds of bone fracture in a given patient compared to non-osteoporosis patients at birth. $\beta_0$ is the log-odds in the reference group, while $\beta_1$ is the increase in risk associated with an osteoporosis diagnosis, and $beta_2$ is the increase associated with a one year increase in age (each holding the other variable steady). The $beta_3$ coefficient is the increase in $beta_2$ given a patient has a prior history of fracture. 
...
rstan identified a prior of $\mu_1$ = `r signif(p_sum$prior$location[1],3)` and $\sigma_1$ = `r signif(sig1,3)` for the priorfrac coefficient. rstan identified a prior of $\mu_2$ = `r signif(p_sum$prior$location[2],3)` and $\sigma_2$ = `r signif(sig2,3)` for the age coefficient. rstan also identified a prior of $\mu_2$ = `r signif(p_sum$prior$location[3],3)` and $\sigma_2$ = `r signif(sig3,3)` for the age coefficient.
