...
mcmc.chain = as.matrix(mod.post)
ci = hdi(mcmc.chain[, "priorfrac.b:age"]); ci
med = median(mcmc.chain[, "priorfrac.b:age"]); med

ci.exp = hdi(exp(mcmc.chain[, "priorfrac.b:age"])); ci.exp
med.exp = median(exp(mcmc.chain[,"priorfrac.b:age"]), na.rm = TRUE); med.exp
...
There is a high probability (low density at 0) of a potentially low magnitude interaction. **Sam or suz put this into better words!!**
...
sensitivity = cs$accuracy_rates[1,1];sensitivity
specificity = cs$accuracy_rates[2,1]; specificity
overall_accuracy = cs$accuracy_rates[3,1]; overall_accuracy

prevalence = mean(pred_data$priorfrac.b == 1); prevalence

## calculate positive predictive value (PPV) using Bayes' theorem
ppv = sensitivity * prevalence / (sensitivity * prevalence + (1 - specificity) * (1 - prevalence)); ppv

cat("The positive predictive value from Bayes' theorem is", signif(ppv, 2))
```
...
cat("The sensitivity is: ", signif(cscv$cv$sensitivity, 2), "\nThe specificity is: ", signif(cscv$cv$specificity,2), "\nThe overal accuracy is:",  signif(cscv$cv$overall_accuracy,2))
