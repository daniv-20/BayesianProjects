...
The mean, and median, because the prior is symmetrical, of $\beta_1$ is `r mu` with a 95% CI of `r signif(ciLower, 2)` to `r signif(ciUpper, 2)`. In other words, the average expected increase in log odds of fracture associated with having a prior history of fracture is `r mu`, with a 95% credible interval of`r signif(ciLower, 2)` to `r signif(ciUpper, 2)`.
